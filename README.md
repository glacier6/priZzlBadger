## ZZL写在最前：
### (1) 按分支来看（各个分支会存在包含关系）   
  NOTE:2025111800 DB初始化
  NOTE:2025111801 读写事务（在初始化的时候会开启一个写线程专门处理写请求）
  NOTE:2025111802 只读事务（注意遍历器也是到这）
  NOTE:2025111803 通用迭代器的初始化
...
zzlTODO:将叶子节点扁平化
zzlTODO:要不要构造key队列,貌似目前来看,就算直接同步的操作,响应延迟也基本不变?
  开一个队列貌似会有很大的额外开销,比如如果开队列但是读的时候不开协程的加入(同步加入),那么可能会抢队列的锁导致阻塞?
  而如果开队列又开协程,那么同时开一大堆协程貌似也不太好,毕竟开协程本身也是要代价的!!
...
### (2)BadgerDB的一些需要关注点
  ...
  - 5.badger 中与事务相关的结构体包括 Txn 和 oracle 两个  
    ...
    NOTE:读时间戳是在创建事务的时候获取的，为o.nextTxnTs-1。之后再执行事务完成之后，提交的时候，再获取提交时间戳，为o.nextTxnTs，且获取之后o.nextTxnTs++
  ...
  - 14.合并时，文件大小上限是SST文件大小上限吗？如果不是，那么如何控制SST的大小上限？
    NOTE:2025121500压缩处计算的文件大小上限各层.sst文件的大小上限，这里的一个.sst文件对应于LSM概念上的那个SSTable范围。

  - 15.已知合并时上层的SST是选取几个，那么这个选取是随机选取吗？
    首先需要知道的是，合并时先选层级（主要的指标是看当前层大小/层目标大小），
    得到溢出层后，在溢出层选取SST时，是依照MAXVersion递增的顺序遍历判断选取的，即先合并旧表。NOTE:2025121501
...
### (5)mmap是什么？及其特性，关键点 
  ...
  与传统IO模式相比，传统方式为 磁盘 → 内核缓存（即内核用的内存空间） → 用户内存（多这个是因为用户的进程无法访问内核级缓存） ，而MMAP为 磁盘 → 内核缓存。减少了一次用户态copy到内核态的操作！！！也因此认为其比较快。    
  ...
  - 6.注意mmap默认的读取单位也是页大小，即4KB，但是可能会触发操作系统的预读取操作，即一次读出来很多个块。  
...
### (12)BadgerDB的加解密
  - 1.NOTE:DGraph对于UID列表加解密那里？是在哪里？ (实际也不算是加解密把,更像是一个压缩空间的过程)
      在Dgraph项目的NOTE:2025121800，分两次解密。
  - 2.NOTE:而BadgerDB的block的加解密在哪里？
      解密在NOTE:2025121803(注意解压缩也在这里),加密在NOTE:2025121804,用的方式是 AES 加密算法 配合 CTR (Counter) 模式来对block加解密。
  - 3.AES加密算法
      AES (Advanced Encryption Standard) 是目前世界上最流行的对称加密算法。
      AES的分组密码 (Block Cipher)是最关键的概念,即 AES 不能直接处理源源不断的数据流，它只能处理固定长度的数据块,一般AES的块大小固定为 128 位 (16 字节),无论你的密钥是 128、192 还是 256 位，AES 每次只能吞吐 16 字节的数据。
      AER就是通过将输入的大小是(16字节 * n)的初始数据经过多轮的替换（把字节A换成字节B）、置换（把位置1移到位置5）、混合（矩阵运算）和异或等操作加密成数据量大小差不多的密文
  - 4.AES加密前后数据量的对比分情况,
      在 AES-CTR 模式下,数据量严格相等。
      在 AES-CBC / ECB 等传统模式下,密文通常比明文稍大（因为需要填充16字节）。就算刚好16字节倍数,通常也要填充一个新的 16 字节块（为了防止解密歧义）。
      在 AES-GCM 等认证加密模式下,密文比明文大（因为包含验证标签）。PS:这个模式就时加了一个防篡改的标签
  - 4.AES+CTR
      这个模式下最大的特点就是数据不用16字节的整数倍数了,所以不用填充了.
      主要就是先有一个IV(初始向量,逐个密钥流块会自增)和一个给定的KEY(密钥),这两个加在一起然后由AES算法生成的块作为密钥流的一个密钥流块(如下).
      AES(Key, IV + 0) = 密钥流块 0
      AES(Key, IV + 1) = 密钥流块 1
      AES(Key, IV + 2) = 密钥流块 2
      AES(Key, IV + 3) = 密钥流块 3
      而密钥流的长度与明文的长度相同,得到密钥流后再与明文进行异或操作即可得到最终的秘闻.PS:也就是说不再把明文数据直接放到AES进行加密了!!
      此外,因为(密文=明文⊕密钥流)(明文=密文⊕密钥流),所以加解密可以用同一个函数
