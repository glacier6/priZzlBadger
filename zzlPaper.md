...
TODO:现在需要数学证明或者动态确定flushThreshold和HotRatioThreshold等几个超参数,要注意结合衰减机制!!不要过于衰减导致果实结不出来
TODO:还有最好想一想冷数据的范围如何减去两端的热数据,以尽量减少底层SST的参与数量!!或者最好是在得出底层涉及SST之前得出去掉热点的数据边界,或者如果冷数据过少范围又广(底层涉及较多SST)时,将其原地合并,不往下层去
...
### 创新点
  ...
  - 6.NOTE:生成一个查询记录KV对的热力图，其内又分为管整个历史的全局热力图以及临时热力图。本质上是利用了空间局部性（应该尽量避免涉及数据的永久改变，因为数据的热度是会变化的）
      ...
      NOTE:下面这两个写冲突的比较有价值做试验！要结合HyperLogLog (HLL)这个数据结构来判断冲突率，看起来比较好用！注意HLL虽然无法分裂，但是可以合并 TODO:但是现在如果要用HLL的话，貌似必须得子节点不继承写入量才行阿！否则会有问题，难道要再分继承的和新的？
