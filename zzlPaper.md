...
NOTE:2026052800 设置L99层容量大小
NOTE:2026052801 修复badgerDB自身的有关none压缩的bug!!

NOTE:测试了缩小L99层的容量,基本没有什么影响;测试了L98层-L99层有最小的大小限制.也基本没有啥影响;测试了L0-Lbase时,如果头部不满足,那么就尝试直接连续找然后直接写入Lbase,不在L0重写,最终导致了更多的写放大
...
NOTE:写放大？写放大如果下层涉及特别散的话全部都重写？需要看一看中间的完全不涉及的会如何.
注意SST的范围是在生成SST的过程中动态生成的,但是现在的问题就是分流后冷数据的范围并不会更新,所以可能会进行一些完全不必要的边缘写放大.
比如如果上层key范围是1-10（只有1和10有数据，10为热点数据），底层涉及的SST范围为1-4，7-8，10会去hotTier，那么冷层的SST最终会生成1-8的一个范围!这就会造成7-8的不必要写放大!
1.一个解决办法就是imemtable刷入L0的时候就分流,但是这个会减慢刷入的速度!可能会减慢写的速度,而且我的衰减和更新快照都是在mem转imem时触发的,此时做分流只会用老版本的快照!
2.另一个解决办法,就是先分流,再计算底层涉及的SST?但是貌似需要将top层的先遍历一遍,此时会造成极大的读放大和CPU放大!
...
NOTE:NOTE:注意BoldDB的版本倒挂不存在，因为他的VLog的回收机制是反向的，即在合并的过程中将判断当前KV的V是否存在高垃圾率的VLog，如果是，就将该V重写到新的Vlog中。此外为了避免冷数据不参与合并，进而导致Vlog不会百分百垃圾回收率，BoldDB会隔一段时间强制归并这些旧SST
...
zzlTODO:优化的手段测试：降低热层的假阳 或 热层直接不压缩 或 二者均有
zzlTODO:树的高度如何才会上升啊，现在不会上升了。。。 
  （1）加大单key大小
  （2）去掉写入的那次读

zzlNOTE:热点转移导致的墓碑倒挂（主要是新版墓碑在下面，而旧版数据在上面有问题，此时新版墓碑可能在合并的过程中被丢；而旧版墓碑在上面，新版数据在下面问题不大，因为BadgerDB不是读到就停的）的问题不能通过简单的overlap判断解决，墓碑倒挂只能通过双写来解决了，即对墓碑分流的时候判断当前key在另一个轨道上是否有sst的范围包含它，如果有，那么需要双写,NOTE:现在是直截了当的双写
...
### 一些系统常识
  ...
  - 3.Cpu Cache Line大小? 
    LInux系统可 etconf LEVEL1_DCACHE_LINESIZE 查看! 一般主流生产环境都是64字节!
...
### 创新点
  ...
  - 5.ABANDON:已知LSM-Tree的数据除了压缩不会改变，而GC是逐层遍历去找当前KV是否仍然有效，那么是否可以用布隆过滤器来对每次压缩的当前层生成一个编码，
      然后用编码判断当前KV是否还存在于某一层（就算假阳性也无所谓，概率太低，所以多余的数据也不会多太多）
      PS：目前布隆应用的对象是SST，所以这里的意思就是把布隆应用在level，而布隆应用的对象可以是key+版本号
      PS：NOTE:现在需要考虑的是对整层布隆的代价大不大（可以增大误判率来减少代价）对整层布隆在BadgerDB上不太好实现,而且代价略大,且之前已经有研究做过类似的了
  ...
  - 6.NOTE:生成一个查询记录KV对的热力图，其内又分为管整个历史的全局热力图以及临时热力图。本质上是利用了空间局部性（应该尽量避免涉及数据的永久改变，因为数据的热度是会变化的）
      ...
      NOTE:NOTE:对于分裂，应该高层的（或者全部）都应该每层以分裂5个或者以上的范围为目标（除非前缀不同的个数不足5个），这样比较好查询
      ...
      NOTE:用法4(a)：（内存中的block都是解压缩过的,一般这种前缀局部性的会导致极高的缓存命中率,所以这个改了之后的针对点不在读取时,而更多的是在层级合并!因为层级合并是需要读取解压缩的!）在选择压缩方法的时候，如果是很热的SST，那么就不压缩，以换取极大的读速。如果是冷的，那么就用更大的压缩比算法.NOTE:关键问题是如何应对热点漂移!!,
      ...
      NOTE:下面这两个写冲突的比较有价值做试验！要结合HyperLogLog (HLL)这个数据结构来判断冲突率，看起来比较好用！注意HLL虽然无法分裂，但是可以合并 NOTE:现在用HLL，子节点继承写入量,导致的问题会随着纪元更迭而消失(因为每次分裂就触发一次纪元更迭,纪元更迭时本来就要清空HLL,且纪元写入量重新计数)
      NOTE:用法3(b)：根据全局热力图对Vlog也分热Vlog和冷Vlog（这个可看看如何只看写热的区域），写入的时候看热力图，重写Vlog时再根据全局热力图更新各个KV的归属
      NOTE:NOTE:用法7(b)：覆写率有两种构造方法,一种就是每个叶子节点挂载上一个HyperLogLog,一种就是在热力树上统计合并时丢弃的key
      PS:注意一些参数要有严格的数学推导!

        PS：注意因为GC是可以看到每个VLOG文件中垃圾的含量的，所以对比比较方便！
      PS：a标识只需要读热分裂即可，b标识需要还需要写热分裂 NOTE:所以，现在可以得知，只需要热读分裂就可以完成大部分功能！！写热当作辅助判断！
      PS：热力图最好是分段线性回归函数，因为SST是范围的原因，感觉最好用直接key的，不要用范围的
      NOTE:PS: 注意一般键值数据库是盲写的,区分不出来UPDATE和WRITE,所以不能通过热力树统计出来每个KEY域内的KV对个数
      
      NOTE:用法7:看看怎么识别出来覆写很热的那块区域,如果能识别出来,那就可以将其圈禁在L0或者L1层,这样就可以避免很多写放大(哪怕是Base层一般是4层或5层,这也减少很多写放大了哦,毕竟base到base+1也需要写放大!),实际这个和上面的用法3(b)是一个道理哦!
      PS:这个覆写率貌似还会有很多用法,可以考虑一下!!
      ...
      TODO:注意，一定要按照读写比来分读热和写热区域和高冲突区域，现在最重要的是怎么判断出这三类区域！
...
- 3.Bf-Tree: A Modern Read-Write-Optimized Concurrent Larger-Than-Memory Range Index
  ...
  - 5.LSM-Community_ A Graph Storage System Exploiting Community Structure in Social 
    ...
    - 7.SharK: Enabling High-Performance Range Queries in Key-Value Store Through Vlog Resharding（注意仅是SCI4区的）
      ...
      - 11.TrieKV
        <!-- 这篇文章也是通过构造一个数据结构来识别热读区域还有热写区域，但是更大的重点是将不同类别的数据方在不同的硬件中（他这篇应用到了新版的那个非易失内存的硬件） -->
        <!-- 
          总共分为三类数据 
          1.读密集型热区
            Read 占比 > 80% 或 90%才会判别为热读区域，将它们放置在 PMem，TrieKV 会给这部分区域赋予最高的压缩优先级。系统会不遗余力地在后台把这些区域的数据压实、排序得整整齐齐（即使这会带来一定的写放大）。
          2.写密集型热区
            强制将它们拘留在 DRAM（普通内存） 的写缓冲区（Write Buffer）中，严禁它们直接写入 PMem。PS：不会丢数据的，因为写入memtable之前会写入WAL中
          3.冷数据区
            访问频次低于自适应阈值，或者曾经是热点但随着 Epoch 时间衰减“失宠”的数据，统统被下放到最底层、最廉价的 SSD（固态硬盘） 中。
        -->
        <!-- 这篇也有时间衰减，他是通过将Trie树上的热度计数器“右移一位”（即除2）做到的 -->

        <!-- 所以说，TrieKV实际上就是根据前缀树(ART,原来是用来存数据用的一个结构)统计出来热的范围（是逐字节统计的，而且前缀树的叶子节点就是一个个KV对，会指向存在DRAM或者PMem内的对应KV对），然后根据这些范围成立一个个桶，然后每个桶负责一个key范围域，每个桶会有其DRAM区域、PMem区域、SSD区域（这里面每个是一个LSM-Tree），而且在SSD里面，层级合并的时候会将读热区域的LSM-TREE提高层级合并的频率，写热区域的LSM-TREE放缓层级合并(避免浪费计算资源) -->
        <!-- 特别需要注意的是,他根据键的范围开启了很多个LSM-TREE,而这些LSM-TREE存储结构还要随着热点范围的合并与分裂同步进行合并和分裂, 这也会带来很大的开销!!但貌似是因为每个桶内的数据不会太大,而且是采用异步替换旧桶(前台不阻塞用户请求),所以性能还可以??-->
        <!-- 特别,NOTE:本文并没用采用通用的平铺压缩(除了L0层的SST,其余层内SST之间的KEY范围均不相交,低读放大高写放大),而用了分级压缩(任一层内各SST的Key范围均可相交,高读放大低写放大) -->

      - 12.Characterizing, Modeling, and Benchmarking RocksDB Key-Value Workloads at Facebook 20年FAST
        <!-- UDB 场景中常规列族（CFs）键较小且大小高度集中,平均在16到30字节之间.特殊列族（Assoc_2ry）键较大且呈长尾分布,通常大于50字节,还有极少部份的大于1KB -->
        <!-- 将 YCSB 的基准测试结果作为生产指导，可能会产生一些误导性的结果。例如，RocksDB 在生产工作负载下的读取性能将高于我们使用 YCSB 测试的表现。由于其极高的读放放大，YCSB 的工作负载很容易使存储带宽极限饱和。此外，YCSB 基准测试结果估计的写入放大低于实际生产。写入性能可能被高估，也可能导致 SSD 寿命估计错误。我们发现导致如此严重的读取放大和存储写入减少的主要原因是对密钥空间局部性的无知。 -->
        <!-- 如何确定键范围的大小（即键范围中包含的 KV 对数量）是基于键范围建模的一个主要挑战。如果键范围极其巨大，热点 KV 对仍然会分散在一个非常大的范围内。对这些 KV 对的访问仍然可能触发大量的数据块读取。如果键范围非常小（例如，每个范围内只有少量的 KV 对），热点 KV 对实际上会位于不同的键范围中，这会退化为与不考虑键范围的模型相同的局限性。根据我们的调查，当键范围大小接近一个 SST 文件中 KV 对的平均数量时，它可以保留数据块（Data block）级别和 SST 级别的局部性。因此，我们使用每个 SST 文件的平均 KV 对数量作为键范围的大小。 -->
        <!-- 基于这些模型，我们进一步使用 db_bench 开发了一个新的基准测试。在运行基准测试时，QPS 模型控制两个连续查询之间的时间间隔。发出查询时，查询类型由从收集到的真实工作负载中计算出的每种查询类型的概率来决定。然后，Key 大小和 Value 大小由拟合模型中的概率函数决定。接下来，基于每个键范围的访问概率，我们选择一个键范围来响应此查询。最后，根据 KV 对访问计数的分布，选择该键范围内的一个 KV 对，并使用其键来组成查询。通过这种方式，基准测试生成的 KV 查询遵循了预期的统计模型。同时，它更好地保留了键空间局部性。 -->
        <!-- 我们将 ZippyDB 工作负载拟合到提出的模型中（排除了 Delete 删除操作），并构建了一个名为 Prefix_dist 的新基准测试。为了评估基于键范围建模的有效性，我们还实现了另外三种具有不同 KV 对分配方式的基准测试：1) Prefix_random 对键范围热度进行建模，但在每个键范围内随机分布热点和冷门 KV 对；2) 类似于 YCSB，All_random 遵循 KV 对访问计数的分布，但在整个键空间中随机分布 KV 对；3) All_dist 将热键集中在整个键空间中，而不是使用随机分布。所有四个基准测试都实现了与 ZippyDB 相似的压缩率。-->
