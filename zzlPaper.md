...
### 创新点
  ...
  - 6.NOTE:生成一个查询记录KV对的热力图，其内又分为管整个历史的全局热力图以及临时热力图。本质上是利用了空间局部性（应该尽量避免涉及数据的永久改变，因为数据的热度是会变化的）
      ...
      NOTE:用法4(a)：（再考虑）在选择压缩方法的时候，如果是很热的SST，那么就不压缩，以换取极大的读速。如果是冷的，那么就用更大的压缩比算法.NOTE:关键问题是如何应对热点漂移!!
      ...
      NOTE:NOTE:用法7(b)：对于热写的区域（不应该是热写，应该是高冲突区域，分裂时构造出一个冲突指数，可为 蓄水池去重后key的个数\蓄水池总数 ，如果要用这个，则需扩大写蓄水池个数），构造一个内存中的墓碑标记（同理去除共有前缀），可以与前面的用法二联动，这样就不用去查LSM树了
